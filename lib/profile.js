...
function renderLessonHistory(lessons) {
  ...
  const lessonsHTML = lessons.map(lesson => {
    ...
    const badgeClass = lesson.status === 'completed' ? 'badge--approved' : 'badge--pending';
    ...
    return `
      <div class="list-item">
        ...
        <div class="list-item-content">
          <div class="list-item-title">${escapeHtml(lesson.subject || 'Subject')}</div>
          <div class="list-item-subtitle">${date}</div>
        </div>
        <div class="list-item-meta">
          <span class="badge ${badgeClass}">${lesson.status}</span>
        </div>
      </div>
    `;
  }).join('');
...
function renderPayments(payments) {
  ...
  const paymentsHTML = payments.map(payment => {
    ...
    const badgeClass = payment.status === 'succeeded' ? 'badge--approved' : 'badge--pending';
